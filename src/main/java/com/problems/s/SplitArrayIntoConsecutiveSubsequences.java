...
public class SplitArrayIntoConsecutiveSubsequences {
    ...
    public boolean isPossible(int[] nums) {
        ...
        long base = min - 1;
        long[] record = new long[(int) (max - base + 1L)];
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (record[index - 1] > 0) {
                ...
                int i1 = search(nums, flag, i + 1, num + 1);
                int i2 = search(nums, flag, i + 1, num + 2);
                if (i1 >= 0 && i2 >= 0) {
                    flag[i1] = true;
                    flag[i2] = true;
                    record[(int) (num + 2 - base)]++;
                } else {
                    return false;
                }
            }
    ...
    int search(int[] nums, boolean[] flag, int start, int num) {
        for (int i = start; i < nums.length && nums[i] <= num; i++) {
            if (!flag[i] && nums[i] == num) {
                return i;
            }
        }
        return -1;
